...
class ManagementCommandView(APIView):
    ...
    ALLOWED_COMMANDS: List[Dict] = [
        {"name": "list_users", "description": "Lists all users in the system"},
        {
    ...
    def get(self, request):
        return Response({"available_commands": self.ALLOWED_COMMANDS})
    ...
    def post(self, request):
        ...
        if not any(cmd["name"] == command for cmd in self.ALLOWED_COMMANDS):
            return Response({"error": f"Command {command} is not allowed"}, status=400)
        ...
        try:
            ...
            return Response({"success": True, "output": output, "error": error})
